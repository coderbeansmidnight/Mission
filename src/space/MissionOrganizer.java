...
public class MissionOrganizer implements Iterable<Mission>
{
	...
	public void add(Mission mission)
	{
		if (mission.getCost() > ONE_TRILLION){
			if (!expensiveMissions.contains(mission)){
				expensiveMissions.add(mission);
			}
		}
		else{
			if (!cheapMissions.contains(mission)){
	...
	{
		ArrayList<Mission> missions = new ArrayList<>(cheapMissions);
		missions.addAll(expensiveMissions);
		// return iterator();
	...
	private final static Mission[]		TEST_MISSIONS =
	{
		...
		new Mission("Mecury", 5E10f),
		new Mission("Astoria", 6E10f),

		new Mission("Oceania", 5E15f),
		new Mission("Newport", 8.3E17f)
	};
